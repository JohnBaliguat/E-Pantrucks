...
function build_rv_route($location)
{
    ...
    // step with php/update/rv.php and php/fetch/get_sku_preview.php.
    $pnbLocations = [
...
if (
    ...
    $pullout_location_arrival_date = validate($_POST["pullout_location_arrival_date"] ?? "");
    $pullout_location_arrival_time = validate($_POST["pullout_location_arrival_time"] ?? "");
    $pullout_location_departure_date = validate($_POST["pullout_location_departure_date"] ?? "");
    $pullout_location_departure_time = validate($_POST["pullout_location_departure_time"] ?? "");
    ...
    $loaded_van_loading_start_date = validate($_POST["loaded_van_loading_start_date"] ?? "");
    $loaded_van_loading_start_time = validate($_POST["loaded_van_loading_start_time"] ?? "");
    $loaded_van_loading_finish_date = validate($_POST["loaded_van_loading_finish_date"] ?? "");
    $loaded_van_loading_finish_time = validate($_POST["loaded_van_loading_finish_time"] ?? "");
    $loaded_van_delivery_departure_date = validate($_POST["loaded_van_delivery_departure_date"] ?? "");
    $loaded_van_delivery_departure_time = validate($_POST["loaded_van_delivery_departure_time"] ?? "");
    $loaded_van_delivery_arrival_date = validate($_POST["loaded_van_delivery_arrival_date"] ?? "");
    $loaded_van_delivery_arrival_time = validate($_POST["loaded_van_delivery_arrival_time"] ?? "");
    $genset_shutoff_date = validate($_POST["genset_shutoff_date"] ?? "");
    $genset_shutoff_time = validate($_POST["genset_shutoff_time"] ?? "");
    $end_uploading_date = validate($_POST["end_uploading_date"] ?? "");
    $end_uploading_time = validate($_POST["end_uploading_time"] ?? "");
    $dr_no = validate($_POST["dr_no"] ?? "");
    $load_description = validate($_POST["load_description"] ?? "");
    $delivered_by_prime_mover = validate($_POST["delivered_by_prime_mover"] ?? "");
    $delivered_by_driver = validate($_POST["delivered_by_driver"] ?? "");
    $delivered_to = validate($_POST["delivered_to"] ?? "");
    $delivered_remarks = validate($_POST["remarks"] ?? "");
    ...
    if ($defect_hubo === "1") {
        // Hour-meter columns are numeric. Never store descriptive text such as
        // "DEFECTIVE" when the unit's Hubo is marked defective.
    ...
    if (empty($segment_empty) || empty($activity_empty)) {
    ...
    } elseif (operations_waybill_exists($conn, $waybill)) {
        $response["message"] = "This waybill number is already in use. Please use a different waybill.";
    } elseif (operations_dr_no_exists($conn, $dr_no)) {
        $response["message"] = "This DR. NO. is already in use. Please use a different DR. NO.";
    } elseif (
        ($e = master_validate_rv(
            $conn, $segment, $activity, $driver, $driver_idNumber,
            $ph, $tr, $gs, $prime_mover,
        )) !== null
    ) {
        $response["message"] = $e;
    } elseif ($skuData === null || $billing_sku === "") {
        $response["message"] = "No SKU found for the selected shipper and PH (Packing House / Location).";
    } else {
        $sql = "INSERT INTO operations (
            ...
            prime_mover, driver, empty_pullout_location,
            ...
            created_by, created_date, \"driver_idNumber\", \"delivered_by_driverIdNumber\"
        ) VALUES (
            ?,?,?,?,?,?,?,?,?,?,
            ?,?,?,?,?,?,?,?,?,?,
            ?,?,?,?,?,?,?,?,?,?,
...
// 66
        ...
        $response["message"] = "RV record created successfully.";
        $response["record"] = [
            "entry_id" => $newId,
