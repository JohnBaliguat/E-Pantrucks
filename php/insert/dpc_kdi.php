...
{
    $ph = strtoupper(trim((string) $ph));

    if ($ph === "") {
        return "DPC KDS-Outside";
    }

    foreach (["PH", "TDC", "DPC"] as $keyword) {
        if (strpos($ph, $keyword) !== false) {
            return "DPC KDS-Compound";
        }
    }
...
// Packing House / Location must be registered (Master Data → Locations).
$__locError = unregistered_locations_message($conn, ["Packing House / Location" => $ph]);
if ($__locError !== "") {
    header("Content-Type: application/json");
...
if (operations_fgtr_no_exists($conn, $fgtr_no)) {
    $response["message"] = "This FGTR's NO. is already in use. Please use a different FGTR's NO.";
    header("Content-Type: application/json");
    echo json_encode($response);
    exit();
}
if (
    ($e = master_validate_dpc(
        $conn, $segment, $activity, $driver, $driver_idNumber, $ph, $tr, $truck,
    )) !== null
) {
    $response["message"] = $e;
    header("Content-Type: application/json");
...
$sql = "INSERT INTO operations (
    ...
    \"18_pads\", \"13_total\", \"18_total\", other_body, other_cover, other_pads, other_total,
    total_load, fgtr_no, remarks, dpc_date, piece_rate, billing_sku, created_by, created_date
) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";

$stmt = $conn->prepare($sql);
$stmt->execute(db_nullable_all([
    $entry_type, $segment, $activity, $waybill_date, $waybill, $evita_farmind,
    $driver, $driver_idNumber, $departure, $arrival, $truck, $tr, $ph,
    $thirteen_body, $thirteen_cover, $thirteen_pads,
    $eighteen_body, $eighteen_cover, $eighteen_pads,
    $thirteen_total, $eighteen_total,
    $other_body, $other_cover, $other_pads, $other_total,
    $total_load, $fgtr_no, $remarks, $dpc_date, $piece_rate, $billing_sku,
    $created_by, $created_date,
]));

$response["success"] = true;
$response["message"] = "DPC KDI entry created successfully.";
$response["record"] = [
