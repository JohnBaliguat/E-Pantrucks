...
if (
    ...
    // Pull-Out / Return must be registered locations (they drive the billing rate matrix).
    ...
    if ($status === "") {
        ...
        if ($action === "add-dry-van") {
            $sql = "INSERT INTO operations (
                entry_type, status, customer_ph, ph, ecs, van_alpha, van_number, shipper,
                eir_out, \"eir_outDate\", \"eir_outTime\", eir_in, \"eir_inDate\",
                pullout_location, pullout_date, pullout_time, delivered_to, return_location, size, commodity, slp_no,
                destination, gs, genset_hr_meter_start, genset_hr_meter_end,
                segment, activity, waybill, date_hauled, driver, truck, tr, tr2,
                ...
                kms, booking, shipment_no, vessel_name, voyage_no, emdr_no, seal, created_by, created_date, \"driver_idNumber\",
            ...
            $response["record"] = [
                "entry_id" => (int) $conn->lastInsertId(),
                "status" => $status, "customer_ph" => $customer_ph, "ph" => $ph, "ecs" => $ecs,
                "van_alpha" => $van_alpha, "van_number" => $van_number, "shipper" => $shipper,
                "eir_out" => $eir_out, "eir_outDate" => $eir_outDate, "eir_outTime" => $eir_outTime,
                "eir_in" => $eir_in, "eir_inDate" => $eir_inDate,
                "pullout_location" => $pullout_location, "pullout_date" => $pullout_date,
                "delivered_to" => $delivered_to, "return_location" => $return_location,
                "size" => $size, "commodity" => $commodity, "slp_no" => $slp_no, "destination" => $destination, "gs" => $gs,
                "genset_hr_meter_start" => $genset_hr_meter_start,
                "genset_hr_meter_end" => $genset_hr_meter_end,
                "segment" => $segment, "activity" => $activity, "driver" => $driver, "truck" => $truck,
                "driver_idNumber" => $driver_idNumber, "driver_return" => $driver_return,
                "truck2" => $truck_return, "driver_return_idNumber" => $driver_return_idNumber,
                "segment_empty" => $segment_empty, "activity_empty" => $activity_empty,
                "waybill" => $waybill, "date_hauled" => $date_hauled, "tr" => $tr, "tr2" => $tr2,
                "date_unloaded" => $date_unloaded, "departure_time" => $departure_time,
                "arrival_time" => $arrival_time, "remarks" => $remarks,
                "waybill_empty" => $waybill_empty, "date_returned" => $date_returned,
                "type" => $type, "delivered_remarks" => $delivered_remarks,
                "booking" => $booking, "shipment_no" => $shipment_no,
                "vessel_name" => $vessel_name, "voyage_no" => $voyage_no, "emdr_no" => $emdr_no, "billing_sku" => $billing_sku,
            ];
        } else {
            $sql = "UPDATE operations SET
                status = ?, customer_ph = ?, ph = ?, ecs = ?, van_alpha = ?, van_number = ?,
                shipper = ?, eir_out = ?, \"eir_outDate\" = ?, \"eir_outTime\" = ?,
                eir_in = ?, \"eir_inDate\" = ?, pullout_location = ?, pullout_date = ?,
                delivered_to = ?, return_location = ?, size = ?, commodity = ?, slp_no = ?,
                destination = ?, gs = ?, genset_hr_meter_start = ?, genset_hr_meter_end = ?,
                segment = ?, activity = ?, waybill = ?, date_hauled = ?, driver = ?,
                truck = ?, tr = ?, tr2 = ?, date_unloaded = ?, departure_time = ?,
                arrival_time = ?, remarks = ?, segment_empty = ?, activity_empty = ?,
                waybill_empty = ?, date_returned = ?, type = ?, delivered_remarks = ?,
                kms = ?, booking = ?, shipment_no = ?, vessel_name = ?, voyage_no = ?, emdr_no = ?, seal = ?, \"driver_idNumber\" = ?,
                driver_return = ?, truck2 = ?, \"driver_return_idNumber\" = ?, billing_sku = ?
                WHERE entry_id = ?";
            ...
            $response["record"] = [
                "entry_id" => $data_id, "status" => $status, "customer_ph" => $customer_ph, "ph" => $ph,
                "ecs" => $ecs, "van_alpha" => $van_alpha, "van_number" => $van_number,
                "shipper" => $shipper, "eir_out" => $eir_out, "eir_outDate" => $eir_outDate,
                "eir_outTime" => $eir_outTime, "eir_in" => $eir_in, "eir_inDate" => $eir_inDate,
                "pullout_location" => $pullout_location, "pullout_date" => $pullout_date,
                "delivered_to" => $delivered_to, "return_location" => $return_location,
                "size" => $size, "commodity" => $commodity, "slp_no" => $slp_no, "destination" => $destination, "gs" => $gs,
                "genset_hr_meter_start" => $genset_hr_meter_start,
