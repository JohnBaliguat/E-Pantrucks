...
<body>
                                ...
                                <div class="row g-3">
                                    ...
                                    <div class="col-md-2">
                                        <div class="mb-3 position-relative">
                                            ...
                                            <input type="text" class="form-control" id="shipper" name="shipper" autocomplete="off" placeholder="Search shipper…" required>
                                    ...
                                    <div class="col-md-6">
                                        <div class="mb-3 position-relative">
                                            ...
                                            <input type="text" class="form-control" id="empty_pullout_location" name="empty_pullout_location" autocomplete="off" required placeholder="Search location">
                                    ...
                                    <div class="col-md-12" hidden>
                                        <label for="genset_hr_meter" class="form-label">GENSET HR METER <span class="text-danger">*</span></label>
                                    ...
                                    <div class="col-md-12" hidden>
                                        <label for="genset_hr_reading" class="form-label">GENSET HR READING <span class="text-danger">*</span></label>
    ...
    <script>
        ...
        function fillFormFromRecord(record) {
            ...
            const dateFields = ['pullout_location_arrival_date', 'pullout_location_departure_date', 'ph_arrival_date', 'delivery_location_arrival_date', 'loading_start_date', 'loading_finish_date', 'delivery_departure_date', 'delivery_arrival_date', 'end_unloading_start_date', 'end_unloading_finish_date', 'gs_start_date', 'gs_end_date', 'waybill_date', 'empty_trip_receipt_date'];
            ...
            if (defectHubo) {
                ...
                if (typeof window.applyDefectHuboState === 'function') {
                    window.applyDefectHuboState();
                }
            }
        }
        ...
        document.addEventListener("DOMContentLoaded", function() {
            ...
            function autofillLastHrMeter(gensetName) {
                ...
                fetch("php/fetch/get_last_hr_meter.php?gs=" + encodeURIComponent(gensetName), { cache: "no-store" })
                    .then(r => r.json())
                    .then(d => { if (d && d.success && d.last_hr_meter != null && d.last_hr_meter !== "") startEl.value = d.last_hr_meter; })
                    .catch(() => {});
            }
        ...
        $(document).ready(function() {
            ...
            function getValues() {
                ...
                const dateFields = ['pullout_location_arrival_date', 'pullout_location_departure_date', 'ph_arrival_date', 
                    ...
                    'end_unloading_start_date', 'end_unloading_finish_date', 'genset_start_date', 'genset_end_date', 'waybill_date', 'empty_trip_receipt_date'];
            ...
            // Expose it so fillFormFromRecord() (defined in an earlier <script> block, outside
            // this DOMContentLoaded scope) can re-apply the state when an edit loads a record.
            window.applyDefectHuboState = applyDefectHuboState;
            ...
            (function () {
                ...
                if (!loadId) return;
                fetch(`php/fetch/get_rv.php?id=${encodeURIComponent(loadId)}`)
                    .then(res => res.json())
                    .then(data => {
                        if (!data.success || !data.record) return;
                        fillFormFromRecord(data.record);
                        const collapseEl = document.querySelector('#entryForm');
                        if (collapseEl) { collapseEl.classList.add('show'); setTimeout(() => collapseEl.scrollIntoView({ behavior: 'smooth', block: 'start' }), 350); }
                    })
                    .catch(() => {});
            })();
                    ...
                    fetch(`php/fetch/get_rv.php?id=${encodeURIComponent(id)}`)
                        .then(res => res.text().then(text => {
                            try {
                                ...
                                // Non-JSON body (e.g. a session-expired HTML page or a server error)
                                throw new Error('The server returned an unexpected response (HTTP ' + res.status + '). You may need to log in again.');
                            }
                        }))
                        .then(data => {
                            if (!data.success) {
                            ...
                            // Separate a form-fill error from a fetch error so it isn't reported as
                            // "Unable to fetch record" when the record actually came back fine.
                            try {
                                fillFormFromRecord(data.record);
                            } catch (fillErr) {
                                Swal.fire('Error', 'Loaded the record but could not display it: ' + fillErr.message, 'error');
                                return;
                            }
                            const collapseEl = document.querySelector('#entryForm');
                            if (collapseEl) {
                        ...
                        .catch(err => {
                            Swal.fire('Error', err.message || 'Unable to fetch record.', 'error');
                        });
                    return;
                }
